...
constexpr char quit = 'x'; // chapter 6 
constexpr char print = ';';
constexpr char name = 'a';
constexpr char let = '#';
const string square = "sqrt";  // chapter 7
const string power = "pow";    // chapter 7
const string declkey = "#";    // chapter 7
const string quitkey = "quit"; // chapter 7
...
double primary(){
	...
	switch (t.kind){
		...
		case 's': // chapter 7 (Square root) 
		{
			...
			else error("The function have to start with (");
			if (ts.get().kind == ')'){
			
			if (SquareRoot >=0)
			return sqrt(SquareRoot);
			
			else error(" the value should be bigger than 0!");
			}
...
cout << "The following operators are available: +,-,*,/,%,sqrt(num),pow(num,num) followed by ;\nYou can finish the program by pressing x or writing quit; \n"; // chapter 6 step 6


	define_name("pi", 3.1415926535);
	define_name("e", 2.7182818284);
	define_name("k", 1000); // chapter 7 step 6
	calculate();
